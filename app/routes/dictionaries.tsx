...
export function meta({}: Route.MetaArgs) {
  return [
    ...
    { name: "description", content: "Select dictionaries and generate VCF files" },
  ];
}
...
function generateVCF(dictionaries: Dictionary[]): string {
  ...
  const vcfContent = uniqueEntries.map(entry => 
...
export default function Dictionaries() {
    ...
    setDictionaries(prev =>
      prev.map(dict => {
        if (dict.name === dictName) {
          ...
          const allEntriesSelected = updatedEntries.every(entry => entry.selected);
          const noEntriesSelected = updatedEntries.every(entry => !entry.selected);
          ...
          return { ...dict, entries: updatedEntries, selected: newDictSelected };
        }
        return dict;
      })
    );
  ...
  const toggleAllEntriesInDict = (dictName: string, select: boolean) => {
    setDictionaries(prev =>
      prev.map(dict => {
        if (dict.name === dictName) {
          const updatedEntries = dict.entries.map(entry => ({ ...entry, selected: select }));
  ...
  const updateReading = (dictName: string, entryId: string, newReading: string) => {
    setDictionaries(prev =>
      prev.map(dict => {
        if (dict.name === dictName) {
          const updatedEntries = dict.entries.map(entry =>
  ...
  const totalSelectedEntries = dictionaries.reduce((sum, dict) => 
    sum + dict.entries.filter(e => e.selected).length, 0
  );
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            ...
            <div className="text-sm text-gray-600">
              {selectedCount} dictionaries selected • {totalSelectedEntries} total entries selected
            </div>
          </div>
            ...
            <div key={category.id} className="space-y-4">
              ...
              <div className="grid gap-4">
                {category.dictionaries.map((dictConfig) => {
                  const dict = dictionaries.find(d => d.name === dictConfig.name);
                  if (!dict) return null;
                    ...
                    >
                      <div className="p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={() => setExpandedDict(
                                expandedDict === dict.name ? null : dict.name
                              )}
                              className="text-blue-600 hover:text-blue-800 text-sm font-medium"
                            >
                              {expandedDict === dict.name ? 'Hide' : 'Show'} entries
                            </button>
                            
                            {expandedDict === dict.name && (
                              <>
                                <button
                                  onClick={() => toggleAllEntriesInDict(dict.name, true)}
                                  className="text-green-600 hover:text-green-800 text-sm font-medium"
                                >
                                  Select All
                                </button>
                                <button
                                  onClick={() => toggleAllEntriesInDict(dict.name, false)}
                              ...
                              {dict.entries.map((entry) => (
                                ...
                                >
                                  <input
                                    ...
                                    onChange={() => toggleEntry(dict.name, entry.id)}
                                    className="h-4 w-4 text-blue-600 rounded border-gray-300"
                                  />
                                  ...
                                  <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    ...
                                    <input
                                      type="text"
                                      value={entry.reading}
                                      onChange={(e) => updateReading(dict.name, entry.id, e.target.value)}
